...
const MathQuizPanel = () => {
  ...
  const score = quizResult
    ? answers.filter((a, i) => a === quizResult.questions[i]?.correctIndex)
        .length
    : 0;
      ...
      >
        ...
        <div className="mqp-header">
          <div>
            ...
            <h2 className="mqp-title">AI QUIZ</h2>
          </div>
            ...
            <div className="mqp-topics">
              ...
              <div className="mqp-topic-grid">
                {topics.map((t) => (
                  <button
                    key={t.apiTopic}
                    className="mqp-topic-btn"
                    onClick={() => handleSelectTopic(t.apiTopic)}
                  >
                    {t.label}
                  </button>
                ))}
          ...
          {phase === "quiz" && currentQuestion && (
            <div className="mqp-quiz">
              <div className="mqp-progress">
                <span className="mqp-progress-label">
                  {currentQ + 1} / {quizResult!.questions.length}
                </span>
                <div className="mqp-progress-bar">
                  <div
                    className="mqp-progress-fill"
                    style={{
                      width: `${((currentQ + 1) / quizResult!.questions.length) * 100}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="mqp-options">
                {currentQuestion.options.map((opt, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="mqp-option-letter">
              ...
              {selected !== null && (
                <button className="mqp-btn" onClick={handleNext}>
                  {currentQ < quizResult!.questions.length - 1
                    ? "Дараах →"
                    : "Дүн харах"}
                </button>
              )}
          ...
          {phase === "result" && quizResult && (
            <div className="mqp-result">
              <div className="mqp-score">
                ...
                <span className="mqp-score-denom">
                  {" "}
