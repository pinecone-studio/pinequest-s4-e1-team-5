...
class RevealMaterial extends THREE.MeshBasicMaterial {
  ...
  get paintUniforms() {
    return {
      uPaintProgress: this._paintProgress,
      uRoomOrigin: this._roomOrigin
    };
  }
  set paintUniforms(v) {
    if (!v) return;
  ...
  onBeforeCompile(shader) {
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <alphatest_fragment>', `#include <alphatest_fragment>
            ...
            `);
    if (hasPaint) {
      const noiseComp = this._paintConfig.noiseAxes === 'xz' ? 'localPos.xz' : this._paintConfig.noiseAxes === 'xy' ? 'localPos.xy' : 'localPos.yz';
