...
import StudioSearchPanel from './StudioSearchPanel';
...
const StudioRoom = ({
  ...
  const handleSearchResultSelect = useCallback(result => {
    const match = monitors.find(item => item.id === result.id);
    if (match) {
      handleMonitorClick(match);
    }
  }, [handleMonitorClick, monitors]);
  ...
  const shouldShowStudioSearch = showRoom && !isWarmup && !isExiting && !isTransitioning && !isTeleporting;
  return <group ref={groupRef} position={[0, -1.2, 0]}>
            ...
            {shouldShowStudioSearch && <Html fullscreen zIndexRange={[1400, 0]}>
                    <div style={{
          position: 'fixed',
          inset: 0,
          pointerEvents: 'none'
        }}>
                        <StudioSearchPanel onSelectResult={handleSearchResultSelect} />
                    </div>
                </Html>}
