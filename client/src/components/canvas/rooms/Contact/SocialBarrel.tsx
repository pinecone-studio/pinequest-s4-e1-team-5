...
const SocialBarrel = ({
  ...
  return <group ref={meshRef} position={position} rotation={rotation} onClick={e => {
    e.stopPropagation();
    onClick && onClick();
            ...
            <mesh ref={paintedRef} position={[0, 0, -0.001]} visible={false}>
                ...
                <meshBasicMaterial color="#e0e0e0" map={texturePainted} transparent={true} alphaTest={0.5} side={THREE.DoubleSide} onBeforeCompile={paintOnBeforeCompile} needsUpdate={!!paintOnBeforeCompile} />
            </mesh>

            {}
            <mesh position={[0, 0, 0]}>
                <planeGeometry args={scale} />
                <revealMaterial color="#e0e0e0" ref={materialRef} map={texture} transparent={true} alphaTest={0.1} uProgress={0.0} paintUniforms={paintUniforms} paintConfig={{
        dirX: 1.0,
        dirY: 0.0,
        dirZ: -0.1,
        startDist: -5.0,
        ...
        noiseAxes: 'yz'
            ...
            {label && <Text ref={textRef} position={[0, scale[1] * 0.26, 0.05]} rotation={[0, 0, 0.03]} fontSize={scale[0] * 0.14} font="/fonts/CabinSketch-Bold.ttf" color="#111111" fillOpacity={paintUniforms ? 0 : 1} anchorX="center" anchorY="middle">
                    {label}
                </Text>}
        </group>;
};
