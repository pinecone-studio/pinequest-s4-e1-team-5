...
interface PeriodicTableOverlayProps {
  onClose: () => void;
}

export default function PeriodicTableOverlay({ onClose }: PeriodicTableOverlayProps) {
  ...
  return (
    <div className="h-full w-full bg-[#020202] relative overflow-hidden text-white font-sans">
      <button
        onClick={onClose}
      ...
      <div className="absolute z-10 w-[95%] md:w-auto px-4 md:px-6 py-3 -translate-x-1/2 border rounded-2xl md:rounded-full bottom-6 md:bottom-10 left-1/2 bg-white/5 backdrop-blur-md border-white/10 overflow-x-auto no-scrollbar">
        <div className="flex items-center gap-6 md:gap-10 min-w-max">
          {[
            { color: 'bg-blue-400', label: 'Метал бус' },
            { color: 'bg-emerald-500', label: 'Шүлтлэг метал' },
            { color: 'bg-red-500', label: 'Газрын шүлтлэг метал' },
            { color: 'bg-yellow-500', label: 'Шилжилтийн метал' },
            { color: 'bg-cyan-500', label: 'Лантонойд' },
            { color: 'bg-pink-600', label: 'Актинойд' },
            { color: 'bg-lime-200', label: 'Метал' },
            { color: 'bg-purple-500', label: 'Хагас метал' },
            { color: 'bg-amber-900', label: 'Инертийн хий' },
          ].map(({ color, label }) => (
            <div key={label} className="flex items-center gap-2 whitespace-nowrap">
              <div className={`w-2 h-2 rounded-full ${color}`} />
              <span className="text-[8px] md:text-[9px] tracking-widest text-gray-400 uppercase font-bold">{label}</span>
            </div>
          ))}
        </div>
      </div>
