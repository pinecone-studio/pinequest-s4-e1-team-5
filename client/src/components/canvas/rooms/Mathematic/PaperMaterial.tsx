  ...
  (
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        get bend() {
          return (
            materialRef.current?.userData?.shader?.uniforms.uBend.value || 0
          );
        },
        set windStrength(value) {
          if (materialRef.current?.userData?.shader) {
            materialRef.current.userData.shader.uniforms.uWindStrength.value =
              value;
          }
        },
        get windStrength() {
          return (
            materialRef.current?.userData?.shader?.uniforms.uWindStrength
              .value || 0
          );
        },
        set uProgress(value) {
          if (materialRef.current?.userData?.shader) {
            materialRef.current.userData.shader.uniforms.uProgress.value =
              value;
          }
        },
        get uProgress() {
          return (
            materialRef.current?.userData?.shader?.uniforms.uProgress.value || 0
          );
        },
    ...
    useFrame((state) => {
      if (materialRef.current?.userData?.shader) {
        ...
        if (shaderHasUniform(materialRef.current.userData.shader, "mapBack")) {
          materialRef.current.userData.shader.uniforms.mapBack.value =
            props.mapBack || null;
        }
        if (
          shaderHasUniform(materialRef.current.userData.shader, "mapPainted")
        ) {
          materialRef.current.userData.shader.uniforms.mapPainted.value =
            props.mapPainted || null;
        }
      }
    });
    const shaderHasUniform = (shader, name) =>
      shader.uniforms && shader.uniforms[name];
    return (
      <meshBasicMaterial
        ref={materialRef}
        map={map}
