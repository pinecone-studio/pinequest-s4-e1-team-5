...
const SocialBarrel = ({
  ...
  const paintedTexturePath = isTouch
    ? texturePath
    : texturePath
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y =
        position[1] + Math.sin(time * 0.8 + phaseOffset) * 0.15;
      meshRef.current.position.x =
        position[0] + Math.sin(time * 0.4 + phaseOffset) * 0.2;
      meshRef.current.rotation.z =
        rotation[2] + Math.sin(time * 0.6 + phaseOffset) * 0.05;
      ...
      if (paintUniforms && textRef.current) {
        ...
        textRef.current.fillOpacity = THREE.MathUtils.clamp(
          pDistFromPlane,
          0,
          1,
        );
