...
const GalleryRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
  ...
  const paintedUrls = UNIQUE_PROJECTS.map((p) =>
    canHover && p.painted ? p.painted : p.front,
  );
  const paintedTextures = useTexture(paintedUrls);
  const backTextureRaw = useTexture(
    canHover
      ? "/textures/gallery/tylkartki_painted.webp"
      : "/textures/gallery/tylkartki.webp",
  );
  const overlayTextureRaw = useTexture(
    canHover
  ...
  const projects = useMemo(() => {
    return Array.from({
      ...
      const techStack = projectData.techStack.map((path) => {
        ...
        if (name === "csslogo")
          return "/textures/gallery/css3logo_painted.webp";
        return `/textures/gallery/${name}_painted.webp`;
      });
      return {
        ...projectData,
      ...
      useFrame((state) => {
        ...
        if (textRef.current && paintProgress) {
          ...
          const expectedOpacity =
            p >= 1.0 ? 1.0 : THREE.MathUtils.clamp((p - 0.3) * 2.0, 0.0, 1.0);
        ...
        if (textRef.current && materialRef.current) {
          ...
          const flutter =
            Math.sin(uTime * 2.0 + y * 2.0) *
            totalWind *
            (1.0 + Math.abs(uBend * 3.0));
          textRef.current.position.z = bendAmount + flutter + 0.02;
          const dz_dy =
            2.0 * y * uBend +
            2.0 *
              Math.cos(uTime * 2.0 + y * 2.0) *
              totalWind *
              (1.0 + Math.abs(uBend * 3.0));
        ...
        if (buttonGroupRef.current && materialRef.current) {
          ...
          buttonGroupRef.current.scale.lerp(
        ...
        const halfWidth = totalWidth / 2;
        let displayX =
          ((((rawX + halfWidth) % totalWidth) + totalWidth) % totalWidth) -
          halfWidth;
        ...
        const wind =
          Math.sin(time * swaySpeed.current + swayOffset.current) * 0.05;
              ...
              <group position={[0, -0.05, 0.01]}>
                {project.techStack &&
                  project.techStack.map((logoPath, idx) => {
                    const spacing = 0.3;
                    const startX =
                      -((project.techStack.length - 1) * spacing) / 2;
