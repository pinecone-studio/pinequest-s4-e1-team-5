...
const CorridorWalls = ({
  ...
  const generateWallSegments = side => {
    ...
    const sideDoors = doorPositions.filter(d => d.side === side).sort((a, b) => b.relativeZ - a.relativeZ);
  ...
  return <group>
      ...
      const uvFraction = SIDE_WIDTH / CENTER_WIDTH;
      const leftSideTexture = floorTexture.clone();
      leftSideTexture.needsUpdate = true;
      leftSideTexture.repeat.set(1, uvFraction);
      ...
      rightSideTexture.offset.set(0, 1 - uvFraction);
      const tiles = [];
      ...
      return <group key={i} position={seg.position} rotation={seg.rotation || [0, seg.rotationY, 0]}>
                            {}
                            <mesh>
