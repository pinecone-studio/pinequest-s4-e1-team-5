...
const Door = ({
  ...
  useFrame(() => {
    ...
    if (glowRef.current) {
      ...
      glowRef.current.material.opacity = THREE.MathUtils.lerp(glowRef.current.material.opacity, targetOpacity, 0.08);
    }
  });
  const doorRotationY = rotationY !== null ? rotationY : side === 'left' ? Math.PI / 2 : -Math.PI / 2;
  return <group position={position} rotation={[0, doorRotationY, 0]} ref={frameRef}>
            {}
            <group position={[0, doorHeight / 2 + 0.5, 0.3]}>
                ...
                {}
                <Text position={[0, 0, 0.01]} fontSize={0.12} color="#1a1a1a" anchorX="center" anchorY="middle" renderOrder={3} depthOffset={-1}>
                    {icon} {label}
                </Text>

                {}
                <Text position={[0, -0.2, 0.01]} fontSize={0.15} color="#39FF14" anchorX="center" renderOrder={3} depthOffset={-1}>
                    ▼
                </Text>
            </group>
            ...
            <mesh position={[0, -0.2, -0.05]} rotation={[0, Math.PI, 0]}>
                ...
                <meshBasicMaterial color="#e0e0e0" transparent={true} opacity={glowIntensity} depthWrite={false} />
            </mesh>
