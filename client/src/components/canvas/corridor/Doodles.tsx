...
const Doodles = () => {
  ...
  return <group ref={groupRef}>
            {}
            <SketchElement texture={textures.paperAirplane} position={[0.5, 0.8, 0.3]} scale={0.55} rotationSpeed={0.15} floatSpeed={0.7} floatAmount={0.04} />

            {}
            <SketchElement texture={textures.paperBall} position={[-0.9, -0.7, 0.4]} scale={0.4} rotationSpeed={0.4} floatSpeed={0.5} floatAmount={0.02} />

            {}
            <SketchElement texture={textures.paperBall} position={[-1.3, 0.5, -0.2]} scale={0.3} rotationSpeed={-0.3} floatSpeed={0.6} floatAmount={0.03} />

            {}
            <SketchElement texture={textures.pencil} position={[0.7, -0.8, 0.5]} scale={0.5} rotationSpeed={0.1} floatSpeed={0.4} floatAmount={0.02} initialRotation={-0.4} />

            {}
            <SketchElement texture={textures.coffeeCup} position={[1.2, 0.6, -0.1]} scale={0.35} rotationSpeed={0.05} floatSpeed={0.35} floatAmount={0.025} />
...
const SketchElement = ({
  ...
  return <group ref={ref} position={position}>
            ...
            <mesh position={[0.01, -0.01, -0.01]}>
                ...
                <meshBasicMaterial color="#e0e0e0" map={texture} transparent={true} opacity={0.15} side={THREE.DoubleSide} depthWrite={false} alphaTest={0.5} />
            </mesh>
            {}
            <mesh>
                <planeGeometry args={[dimensions.width, dimensions.height]} />
                <meshBasicMaterial color="#e0e0e0" map={texture} transparent={true} side={THREE.DoubleSide} depthWrite={false} alphaTest={0.5} />
            </mesh>
        </group>;
};
const AnimatedStar = ({
...
const Squiggle = ({
  ...
  return <group ref={ref} position={position} rotation={[0, 0, rotation]}>
            {[0, 1, 2, 3, 4, 5].map(i => <mesh key={i} position={[i * 0.07, Math.sin(i * 1.5) * 0.035, 0]}>
