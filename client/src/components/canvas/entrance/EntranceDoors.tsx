...
const EntranceDoors = ({
  ...
  const isMobileDevice =
    ...
    (isTouchDevice() || window.innerWidth < 1000);
  const dummyTex =
  ...
  const doorRightPaintedTexture = useTexture(
    isMobileDevice ? dummyTex : "/textures/doors/door_right_painted.webp",
  ); //hereggui
  const doorLeftPaintedTexture = useTexture(
    isMobileDevice ? dummyTex : "/textures/doors/door_left_painted.webp",
  ); //hereggui
  const handleLeftTexture = useTexture(
    "/textures/doors/handle_left_sketch.webp",
  ); //hereggui
  const handleLeftPaintedTexture = useTexture(
    isMobileDevice ? dummyTex : "/textures/doors/handle_left_painted.webp",
  );
  const handleRightTexture = useTexture(
    "/textures/doors/handle_right_sketch.webp",
  ); //hereggui
  const handleRightPaintedTexture = useTexture(
    isMobileDevice ? dummyTex : "/textures/doors/handle_right_painted.webp",
  );
  ...
  const windowSketchTexture = useTexture(
  ...
  const handleClick = (e) => {
    ...
    if (leftHandleRef.current) {
      tl.to(
        leftHandleRef.current.rotation,
        {
          z: 0.4,
          duration: 0.15,
          ease: "power2.out",
    ...
    if (rightHandleRef.current) {
      tl.to(
        rightHandleRef.current.rotation,
        {
          z: -0.4,
          duration: 0.15,
          ease: "power2.out",
  ...
  const handlePointerEnter = () => {
    ...
    if (leftHandlePaintedRef.current)
      leftHandlePaintedRef.current.visible = true;
    if (rightHandlePaintedRef.current)
      rightHandlePaintedRef.current.visible = true;
  };
  const handlePointerLeave = () => {
    ...
    handleHideDelayRef.current = gsap.delayedCall(0.55, () => {
      if (leftHandlePaintedRef.current)
        leftHandlePaintedRef.current.visible = false;
      if (rightHandlePaintedRef.current)
        rightHandlePaintedRef.current.visible = false;
    });
  };
  ...
  useFrame(({ clock }) => {
    if (mousePivotRef.current) {
      mousePivotRef.current.rotation.x =
  ...
  return (
    <group ref={groupRef} position={[position[0], 0, position[2]]}>
      ...
      {}
      <mesh
        position={[-(doorOpeningWidth / 2 + sideWallWidth / 2), wallCenterY, 0]}
      >
      ...
      <group ref={leftDoorRef} position={[-doorWidth, doorCenterY, 0]}>
        ...
        <group
          ref={leftHandleRef}
          position={[doorWidth / 2 + 0.357, -0.099, 0.1]}
        >
          {}
          {!isMobile && (
            <mesh
              ref={leftHandlePaintedRef}
              position={[-0.357, 0.09, -0.001]}
              visible={false}
            >
              <planeGeometry args={[doorWidth, doorHeight]} />
              <meshBasicMaterial
