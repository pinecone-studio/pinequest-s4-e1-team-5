...
export const GALLERY_TEXTURES = [
  ...
  ...GALLERY_TEXTURES_VERSIONED.flatMap((name) => [
    ...
    name === "csslogo"
      ? `/textures/gallery/css3logo_painted.webp`
...
export const PRELOAD_LOADER = [...ABOUT_TEXTURES, ...STUDIO_TEXTURES];
export const filterTexturesByDevice = (list, usePainted) => {
  const paintedVersions = new Set(
    list.filter((p) => p.includes("_painted.webp")),
  );
  ...
  return list.filter((path) => {
    ...
    if (isPainted) {
      ...
      if (
        list.includes(pVersion) ||
        (path.includes("csslogo.webp") && hasCss3Painted)
      ) {
        return true;
      }
      return true;
