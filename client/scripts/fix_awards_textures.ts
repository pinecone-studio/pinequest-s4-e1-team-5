...
const SOTD_FILES = ['SOTDAYYOUNGMULTICSSWINNER.webp', 'SOTDAYYOUNGMULTIDESIGNNOMINESS.webp', 'SOTDAYYOUNGMULTIGSAP.webp', 'SOTDAYYOUNGMULTIORPETRON.webp'];
async function fixAwardsToSquarePOT() {
  ...
  for (const file of SOTD_FILES) {
    ...
    if (fs.existsSync(backupPath)) {
      ...
      await sharp(backupPath).resize(512, 512, {
        fit: 'fill'
      }).webp({
        quality: 90
